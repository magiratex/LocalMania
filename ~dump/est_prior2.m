function xval = est_prior2(x, xhat, Gr)
...
f = @(x) sum(sum((K(1) * create_log_mat(bsxfun(@times, x0, K(2:end))) - x).^2)) + ...
...
fval = sum(sum((K(1) * create_log_mat(bsxfun(@times, x0, K(2:end))) - x).^2)) + ...
         beta * sum(sum((xhat - x).^2));
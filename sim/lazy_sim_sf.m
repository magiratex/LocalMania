...
for t = 1 : T
    ...
    %% update agents
    outcome = zeros(size(conf,1), 4);
    for i = 1 : size(conf, 1)
        helbRes = helbPredict(i, conf(:, 3:end-2), dT, groupSFParam, groupSFParam);
        outcome(i, :) = helbRes;
    end;
    conf(:, 3:6) = outcome;    
...
save(['data_sf_',goalSelectType,'_tradeshowL.mat'], 'agtList');
% save data agtList;
